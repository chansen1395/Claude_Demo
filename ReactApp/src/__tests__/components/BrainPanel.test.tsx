...
import type { BrainVizSize, BrainStatValues } from '../../renderers/brainRenderer';
...
describe('BrainPanel', () => {
  it('renders with null creature', () => {
    const sizeRef = { current: null } as React.MutableRefObject<BrainVizSize | null>;
  ...
  it('renders with a creature that has activations', () => {
    ...
    (renderBrain as ReturnType<typeof vi.fn>).mockReturnValueOnce(mockStats);

    const sizeRef = { current: null } as React.MutableRefObject<BrainVizSize | null>;
    renderWithTheme(<BrainPanel creature={creature} brainSizeRef={sizeRef} />);
    expect(screen.getByText('Neural Network')).toBeInTheDocument();

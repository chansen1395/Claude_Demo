...
export function Controls({
  paused, speed, mutRate, trails,
  onTogglePause, onSpeedChange, onMutRateChange, onToggleTrails, onReset,
}: ControlsProps) {
  const mutPercent = Math.round(mutRate * 100);

  const handleSpeedInput = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => onSpeedChange(Number(e.target.value)),
    [onSpeedChange],
  );

  const handleMutInput = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => onMutRateChange(Number(e.target.value) / 100),
    [onMutRateChange],
  );
    ...
    <FooterBar>
      <ControlsRow>
        ...
        <ControlGroup>
          ...
          <Slider type="range" min={1} max={10} value={speed} onChange={handleSpeedInput} />
          <ControlValue>{speed}×</ControlValue>
        </ControlGroup>
        <ControlGroup>
          <ControlLabel>Mutation</ControlLabel>
          <Slider type="range" min={1} max={30} value={mutPercent} onChange={handleMutInput} />
